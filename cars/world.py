...
class SimpleCarWorld(World):
    ...
    def _draw_ladar(self, sensors, state, scale):
        surface = self.screen.copy() # surface = pygame.display.get_surface().copy()
        surface.fill(white)
    ...
    def _prepare_visualization(self): 
        red = (254, 0, 0)
        pygame.init()
        self.screen = pygame.Surface(self.size) # screen = pygame.display.set_mode(self.size)
        self.screen.fill(white)
        scale = plot_map(self.map, self.screen)
    ...
    def _update_display(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return pygame.QUIT
        display = self.screen # display = pygame.display.get_surface()
        display.fill(white)
