...
def get_line_coefs(p1, p2):
    ...
    for _ in range(10):
        try:
            ...
            #   1) A * p1.real  +  B * p1.imag  +  C * 1    =   0 
            #   2) A * p2.real  +  B * p2.imag  +  C * 1    =   0 
            #   3) A * 1        +  B * 1        +  C * 1    =   1 
...
def intersect_ray_with_segment(ray, segment):
    ...
    if not (min(s_start.imag, s_end.imag) <= intsct.imag <= max(s_start.imag, s_end.imag)):
        return None
    # Check: 
    #        1) if the ray [r_dir.real] and [(intsct - r_start).real] are of the same sign, otherwize they won't intersect
    #        2) .....same for [imag].....
